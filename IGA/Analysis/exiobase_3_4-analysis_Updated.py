...
Y_sum_covid = Y.sum(1)
Y_reduction = Y_sum_covid[4155] * (1-0.60)  # 60% reduction in final demand
Y_sum_covid[4155] = Y_reduction 

# calculate x_covid (Total Output)
Z = A * x
x_covid = Y_sum_covid + Z.sum(1)

# Value Added Covid
V_covid = x_covid - Z.sum(0)

#%% Calculate F_va - DONE INCORRECTLY BUT I THINK f_covid IS CORRECT

# f_covid = F/x_covid
# F_covid = f_covid @ L
# F_va_covid = F_covid[0:22,:]

#%% Scenario where the impact of the NOW package is calculated on the hospitality sector in the Netherlands
# double check approach for NOW1 implementation (Jas)

# Implement Package NOW1 - this calculates the % govt payed by taking into account wages
NOW_1 = F[2:4, 4155].sum() * 0.45

# Calculating final demand including the NOW1 packahe
Y_NOW1 = Y.sum(1)
Y_NOW1[4155] = Y_sum_covid[4155] + NOW_1

# Calculating Total expenditure and Value added with implementation of NOW1 package
x_NOW1 = Y_NOW1 + Z.sum(1)
V_NOW1= x_NOW1 - Z.sum(0)

# Calculating F with the NOW package implementation - DONE INCORRECTLY DOESNT WORK
f_NOW1 = F/x_NOW1
F_NOW1 = f_NOW1 @ L @ Y_NOW1

F_va_NOW1 = F_NOW1[0:22,:]
F_emissions_NOW1 = F_NOW1[23:29,:]

#%% Scenario where the impact of COVID 19 is calculated on the hospitality sector in the Netherlands
#What we did before but with Total Y

Y_covid = mrio['Y_covid']
Y_red = Y_covid[4155,:] * 0.4 # 60% reduction of the final demand of the hotel sector
Y_covid[4155] = Y_red

Y_sum1 = Y_covid.sum(1)

# THIS IS THE SAME AS 'Y_sum_covid' ----- 
